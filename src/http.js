...
export async function fetchCart(){
  const response = await fetch('https://flower-app-b21ff-default-rtdb.europe-west1.firebasedatabase.app/cart.json');
  const data = await response.json();
  if (!response.ok) {
...
export async function updateCart(cart) {
  ...
  if (!response.ok) {
    throw new Error('Ошибка при обновлении состояния корзины');
  }
